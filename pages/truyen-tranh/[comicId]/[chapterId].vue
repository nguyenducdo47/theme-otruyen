...
let {
  ...
  thumb_url,
  is_adult,
} = await useFetchData(`/truyen-tranh/${comicId}/${chapterId}`);

chapters = chapters.sort((a: any, b: any) => +a.name - +b.name);
...
const handleChangeEpisode = (type: 'prev' | 'next') => {
  ...
  const chapterIdx = chapters.findIndex(
    (chapter: any) => chapter.name === chapterId
  );
  const nextChapterIdx = chapterIdx + (type === 'next' ? 1 : -1);
  navigateTo(`/truyen-tranh/${comicId}/${chapters[nextChapterIdx].name}`);
};
...
onMounted(async () => {
  ...
  // const comic: ComicDetail = await useFetchData(`/truyen-tranh/${comicId}`);
  // const { author, slug, status, name, thumb_url, is_adult } = comic;
  historyAddComic({
    author: author[0],
    slug,
    is_adult,
    ...
    last_reading: `Chương ${chapterId.toString()}`,
...
<template>
  <main class="bg-zinc-900 min-h-screen">
    ...
    <div class="fixed inset-0" @click="handleShowToolbar">
      ...
      >
        <NuxtLink :to="`/truyen-tranh/${comicId as string}`">{{
          name
        }}</NuxtLink>
        ...
        <div class="flex items-center gap-3">
          <button
            :class="`px-3 py-1 rounded-full ${
              chapterId == chapters.at(0).name
                ? 'bg-gray-200 text-gray-500'
                : 'bg-emerald-200 text-emerald-500 '
            }`"
            @click="handleChangeEpisode('prev')"
            :disabled="chapterId == chapters.at(0).name"
          >
            Trước
          </button>
          <button
            :class="`px-3 py-1 rounded-full ${
              chapterId == chapters.at(-1).name
                ? 'bg-gray-200 text-gray-500'
                : 'bg-emerald-200 text-emerald-500 '
            }`"
            @click="handleChangeEpisode('next')"
            :disabled="chapterId == chapters.at(-1).name"
          >
            Tiếp
          </button>
          <button
            ...
            >
              ...
              <ul class="overflow-auto text-sm h-max max-h-72 font-normal">
                <NuxtLink
                  ...
                  :to="`/truyen-tranh/${comicId}/${chapter.name}`"
                  :key="chapter.id"
