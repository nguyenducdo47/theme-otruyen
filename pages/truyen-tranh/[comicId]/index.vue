...
const newestChapter = comic.chapters.at(-1)?.name.match(/\d+(\.\d+)?/)?.[0];
...
const onAddDownloadChapter = (chapterId: string) => {
...
<template>
  <div class="relative pt-16 px-4 min-h-screen">
    <div
    ...
    >
      ...
      <div class="sm:col-span-3">
        ...
        <div class="font-semibold flex items-center gap-2 my-1">
          ...
          <template v-else-if="comic.author === 'Updating'">
        ...
        <div class="mt-2" v-if="comic.content">
          <p
            ...
            ref="description"
          >
            {{ comic.content.replace(/\<p\>/g, '').replace(/\<\/p\>/g, '') }}
          </p>
          <button
          ...
          >
            {{ showFullDescription ? 'Rút gọn' : 'Thêm' }}
          </button>
        </div>
        <div
          ...
          <button
            @click="
              () => {
                ...
                navigateTo(
                  `/truyen-tranh/${comic.slug}/${comic.chapters[0]?.name}`
                );
              }
            "
      ...
      <div v-show="currentTab === 'chapters'">
        ...
        <ul class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <NuxtLink
            ...
            :to="`/truyen-tranh/${comic.slug}/${chapter.name}`"
          >
            <abbr :name="chapter.name" class="no-underline">
              {{ chapter.name }}
            </abbr>
          </NuxtLink>
        </ul>
  ...
  >
    ...
    <div class="bg-white rounded-lg py-4 px-6 w-[90vw] max-w-3xl">
      <div class="flex flex-col sm:flex-row items-center gap-2.5 sm:gap-5">
        <h3 class="text-2xl font-semibold">Chọn chương</h3>
        <div
      ...
      >
        <li
          ...
          :class="`border rounded px-2 py-1 cursor-pointer duration-100 truncate ${
            downloadChapters.includes(chapter.name)
              ? 'border-emerald-500 bg-emerald-500 text-white'
              : ''
          }`"
          @click="onAddDownloadChapter(chapter.name)"
        >
          {{ chapter.name }}
        </li>
      </ul>
