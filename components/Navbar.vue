...
const handleSearchComics = () => {
  ...
  searchInput.value.blur();
  navigateTo(`/tim-kiem?q=${searchValue.value.replace(/\s+/g, '+')}`);
};
...
<template>
  <header class="shadow bg-white fixed z-50 top-0 w-full">
    <nav
        ...
        >
          ...
          >
            <li
              ...
              :key="comic.slug"
              @mousedown="handleSelectComic(comic.slug)"
            ...
            >
              ...
              <div>
                ...
                >
                  <template v-if="comic.author === 'Updating'">
                ...
                <p class="text-xs font-semibold flex items-center">
                  <template v-if="Array.isArray(comic.category)">
                    {{ comic.category.map((c: any) => c.name).join(', ') }}
                  </template>
                </p>
              </div>
            </li>
