...
const handleClickCard = (e: Event, type: 'detail' | 'delete' | 'continue') => {
  e.stopPropagation();
  if (type === 'delete') {
    historyDeleteComic(slug);
    emit('delete-comic', slug);
    return;
  }
  if (type === 'continue') {
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <abbr :title="name" class="no-underline">{{ name }}</abbr>
      </h5>
      <template v-if="detail">
        <hr class="mt-3 mb-0.5 border-gray-500" />
        <div>
          <p class="text-sm text-gray-300 truncate font-semibold">
            <template v-if="Array.isArray(author)">
              {{ author.join(' | ') }}
            </template>
            <template v-else-if="author === 'Updating'">
